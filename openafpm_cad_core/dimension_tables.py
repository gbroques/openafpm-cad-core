...
__all__ = ["load_dimension_tables", "get_dimension_tables"]
...
def load_dimension_tables(
    ...
    alternator_document = load_alernator(recompute_all=True)
    return get_dimension_tables(
        spreadsheet_document, alternator_document, img_path_prefix
    )


def get_dimension_tables(
    spreadsheet_document: Document,
    alternator_document: Document,
    img_path_prefix: str = "",
) -> List[Element]:
    ...
    tables = []
    tables.append(create_dimension_of_hub_plywood_pieces_table(spreadsheet_document))
    tables.append(create_yaw_bearing_pipe_sizes_table(spreadsheet_document))
    tables.append(create_wheel_bearing_hub_table(spreadsheet_document))
    tables.append(create_steel_disk_sizes_table(spreadsheet_document))
    tables.append(create_frame_dimensions_table(spreadsheet_document, img_path_prefix))
    tables.append(create_offset_table(spreadsheet_document))
    if wind_turbine_shape == WindTurbineShape.T:
        ...
        tables.append(
            create_frame_dimensions_flat_bar_table_top_view(
                spreadsheet_document, img_path_prefix
            )
        )
        tables.append(
            create_frame_dimensions_flat_bar_table_side_view(
                spreadsheet_document, img_path_prefix
            )
        )
    tables.append(
        create_steel_pipe_dimensions_for_tail_table(
            spreadsheet_document, img_path_prefix
        )
    )
    tables.append(
    ...
    tables.append(create_magnets_and_coils_table(spreadsheet_document))
    tables.append(
    ...
    tables.append(create_various_parts_dimensions_table(spreadsheet_document))
    tables.append(
        create_total_pipe_length_by_outer_diameter_table(spreadsheet_document)
    )
...
def create_table(
    ...
    img_style: Optional[str] = None,
) -> Element:
    col_span = 3 if img_src_and_alt else 2
    children = [
        thead([tr([th(header, col_span=col_span)])]),
        tbody([tr([td(row[0]), td(row[1])]) for row in rows]),
    ]
    if img_src_and_alt:
    ...
    if footer_rows:
        children.append(
            tfoot([tr([td(row, col_span=col_span)]) for row in footer_rows])
        )
...
def create_dimension_of_hub_plywood_pieces_table(
    spreadsheet_document: Document,
) -> Element:
    return create_table(
        ...
        [
            (
                "Thickness",
                round_and_format_length(
                    spreadsheet_document.Blade.BladeAssemblyPlateThickness
                ),
            ),
            (
                "Disk diameter",
                round_and_format_length(
                    spreadsheet_document.Blade.BladeAssemblyBackDiskDiameter
                ),
            ),
            (
                "Triangle side",
                round_and_format_length(
                    spreadsheet_document.Blade.BladeAssemblyFrontTriangleSideLength
                ),
            ),
            (
...
def create_frame_dimensions_table(
    spreadsheet_document: Document, img_path_prefix: str = ""
) -> Element:
    ...
    if wind_turbine_shape == WindTurbineShape.T:
        return create_table(
            ...
            [
                ...
                (
                    "Channel pieces B,C",
                    round_and_format_length(spreadsheet_document.Alternator.BC),
                ),
                (
                    "End bracket D",
                    round_and_format_length(spreadsheet_document.Alternator.D),
                ),
                (
                    "Position of shaft X",
                    round_and_format_length(spreadsheet_document.Alternator.X),
                ),
                *steel_angle_section_rows,
            ],
        ...
        return create_table(
            ...
            img_src_and_alt=(
                img_path_prefix + "star-shape-frame.png",
                "Star shape frame",
            ),
        )


def create_alternator_frame_to_yaw_pipe_sizes_table(
    spreadsheet_document: Document, img_path_prefix: str = ""
) -> Element:
    return create_table(
        ...
        img_src_and_alt=(
            img_path_prefix + "t-shape-yaw-bearing-frame-junction.png",
            "T shape yaw bearing frame junction",
        ),
    )
...
def create_frame_dimensions_flat_bar_table_top_view(
    spreadsheet_document: Document, img_path_prefix: str = ""
) -> Element:
    return create_table(
        ...
        [
            (
                "Offset",
                round_and_format_length(spreadsheet_document.Spreadsheet.Offset),
            ),
            (
                "L length of flat bar",
                round_and_format_length(spreadsheet_document.YawBearing.L),
            ),
            (
                "M width of flat bar",
                round_and_format_length(spreadsheet_document.YawBearing.MM),
            ),
        ],
...
def create_frame_dimensions_flat_bar_table_side_view(
    spreadsheet_document: Document, img_path_prefix: str = ""
) -> Element:
    return create_table(
        ...
        [
            ...
            (
                "P side piece flat bar length",
                round_and_format_length(spreadsheet_document.YawBearing.SideLength),
            ),
            (
                "Q side piece flat bar width",
                round_and_format_length(spreadsheet_document.YawBearing.SideWidth),
            ),
            (
                "R side piece flat bar thickness",
                round_and_format_length(
                    spreadsheet_document.Spreadsheet.FlatMetalThickness
                ),
            ),
            (
                "S length from top of channel section to weld flat bar",
                round_and_format_length(
        ...
        img_src_and_alt=(
            img_path_prefix + "side-of-extended-frame.svg",
            "Side of extended frame",
        ),
    )


def create_steel_pipe_dimensions_for_tail_table(
    spreadsheet_document: Document, img_path_prefix: str = ""
) -> Element:
    return create_table(
        ...
        [
            (
                "Boom length A",
                round_and_format_length(spreadsheet_document.Spreadsheet.BoomLength),
            ),
            (
                "Diameter B",
                round_and_format_length(
            ...
            (
                "Hinge outer C",
                round_and_format_length(spreadsheet_document.Tail.HingeOuterPipeLength),
            ),
            (
                "Diameter D",
                round_and_format_length(
            ...
            (
                "Hinge inner E",
                round_and_format_length(spreadsheet_document.Tail.HingeInnerPipeLength),
            ),
            (
                "Diameter F",
                round_and_format_length(
...
def create_tail_vane_dimensions_table(
    spreadsheet_document: Document, img_path_prefix: str = ""
) -> Element:
    ...
    return create_table(
        ...
        [
            ...
            (
                "Vane plywood dimension G",
                round_and_format_length(spreadsheet_document.Spreadsheet.VaneWidth),
            ),
            (
                "Vane plywood dimension H",
                round_and_format_length(spreadsheet_document.Spreadsheet.VaneLength),
            ),
            (
                "Vane plywood thickness",
                round_and_format_length(spreadsheet_document.Spreadsheet.VaneThickness),
            ),
            (
        ...
        [
            ...
            "Vane bracket bolts, nuts, and washers are stainless steel",
        ],
...
def create_magnets_and_coils_table(spreadsheet_document: Document) -> Element:
    return create_table(
        ...
        [
            (
                "Number of rotor disks",
                get_number_of_rotors(spreadsheet_document.Spreadsheet.RotorTopology),
            ),
            ...
            (
                "Magnet length",
                round_and_format_length(spreadsheet_document.Spreadsheet.MagnetLength),
            ),
            (
                "Magnet width",
                round_and_format_length(spreadsheet_document.Spreadsheet.MagnetWidth),
            ),
            (
                "Magnet thickness",
                round_and_format_length(
            ...
            (
                "Number of coils",
                spreadsheet_document.Spreadsheet.NumberOfCoilsPerPhase * 3,
            ),
            (
                "Weight of wire",
                round_and_format_weight(spreadsheet_document.Spreadsheet.WireWeight),
            ),
            (
                "Wire diameter",
                format_length(spreadsheet_document.Spreadsheet.WireDiameter),
            ),
            (
...
def create_tail_junction_dimensions_table(
    spreadsheet_document: Document, img_path_prefix: str = ""
) -> Element:
    return create_table(
        ...
        img_src_and_alt=(
            img_path_prefix + "tail-hinge-junction-cover-top-dimensions.svg",
            "Width of cross piece D",
        ),
    )


def create_coil_winder_dimensions_table(
    spreadsheet_document: Document, img_path_prefix: str = ""
) -> Element:
    coil_type = spreadsheet_document.Spreadsheet.CoilType
    if coil_type == 1:
        img_src_and_alt = (
            img_path_prefix + "rectangular-coil-winder-dimensions.svg",
            "Rectangular coil winder dimensions",
        )
    elif coil_type == 2:
        img_src_and_alt = (
            img_path_prefix + "keyhole-coil-winder-dimensions.svg",
            "Keyhole coil winder dimensions",
        )
    else:
        img_src_and_alt = (
            img_path_prefix + "triangular-coil-winder-dimensions.svg",
            "Triangular coil winder dimensions",
        )
    return create_table(
        "Coil Winder Dimensions",
        [
            (
                "Cheek piece diameter",
                round_and_format_length(
            ...
            (
                "A",
                round_and_format_length(
                    spreadsheet_document.Alternator.RectangularVerticalDistanceOfHolesFromCenter
                    * 2
                ),
            ),
            (
            ...
            (
                ...
                round_and_format_length(
                    ...
                    if spreadsheet_document.Spreadsheet.CoilType != 3
                    else spreadsheet_document.Alternator.CoilWinderDiskBottomHoleRadius
                    * 2
                ),
            ),
            (
                "Threaded rod diameter",
                f"M{round(spreadsheet_document.Alternator.CoilWinderCenterRodDiameter)}",
            ),
            (
                "Pin diameter",
                f"M{round(spreadsheet_document.Alternator.CoilWinderPinDiameter)}",
            ),
        ],
...
def create_stator_mold_dimensions_table(
    spreadsheet_document: Document, img_path_prefix: str = ""
) -> Element:
    rotor_disk_radius = spreadsheet_document.Spreadsheet.RotorDiskRadius
    wind_turbine_shape = WindTurbineShape.from_string(
        ...
        [
            ...
            (
                ...
                round_and_format_length(
                    ...
                    if wind_turbine_shape != wind_turbine_shape.STAR
                    else spreadsheet_document.Alternator.HexagonalStatorOuterCircumradius
                ),
            ),
            (
                "Inner radius C",
...
def create_rotor_mold_dimensions_table(
    spreadsheet_document: Document, img_path_prefix: str = ""
) -> Element:
    number_of_rotors = get_number_of_rotors(
        spreadsheet_document.Spreadsheet.RotorTopology
    )
    return create_table(
        ...
        [
            ...
            (
                "Island radius C",
                round_and_format_length(spreadsheet_document.Alternator.IslandRadius),
            ),
            (
                "Surround thickness",
                round_and_format_length(
...
def create_magnet_positioning_jig_dimensions_table(
    spreadsheet_document: Document, img_path_prefix: str = ""
) -> Element:
    rows = [
        ...
        (
            "Larger radius E",
            round_and_format_length(spreadsheet_document.Spreadsheet.RotorDiskRadius),
        ),
        (
            "Circle radius",
            round_and_format_length(spreadsheet_document.Spreadsheet.MagnetWidth / 2),
        ),
        (
            "Bolts (fully threaded)",
            format_fastener(
    ...
    return create_table(
        ...
        (
            img_path_prefix + "magnet-positioning-jig-dimensions.png",
            "Magnet positioning jig dimensions",
        ),
    )
...
def create_total_pipe_length_by_outer_diameter_table(
    spreadsheet_document: Document,
) -> Element:
    pipe_outer_diameter_length_tuples = get_pipe_outer_diameter_length_tuples(
        spreadsheet_document
    )
    pipe_outer_diameter_total_length_tuples = sum_length_by_diameter(
        pipe_outer_diameter_length_tuples
    )
    rows = [
...
def create_studs_nuts_and_washers_table(spreadsheet_document: Document) -> Element:
    ...
    studs_diameter_length_tuples = get_studs_diameter_length_tuples(
        spreadsheet_document
    )
    studs_diameter_total_length_tuples = sum_length_by_diameter(
        studs_diameter_length_tuples
    )
    rows = [
    ...
    return create_table(
        ...
        [
            ...
            (
                "Rotor disk assembly washers (standard)",
                format_fastener(
            ...
            (
                "Stator assembly washers (standard)",
                format_fastener(
            ...
            (
                "Jacking hole diameter",
                format_length(spreadsheet_document.Alternator.JackingHoleDiameter),
            ),
            (
                ...
                format_fastener(
                    ...
                    + estimated_coil_winder_handle_length,
                ),
            ),
            *rows,
        ],
...
def create_resin_table(
    spreadsheet_document: Document, alternator_document: Document
) -> Element:
    rotor_disk_radius = spreadsheet_document.Spreadsheet.RotorDiskRadius
    wind_turbine_shape = WindTurbineShape.from_string(
        spreadsheet_document.Spreadsheet.CalculatedWindTurbineShape
    ...
    weight_of_resin = (
        total_resin_volume * vinyl_ester_resin_density * resin_weight_scale_factor
    )
    return create_table(
        "Resin",
        [
...
def sum_length_by_diameter(
    diameter_length_tuples: List[Tuple[float, float]],
...
def get_pipe_outer_diameter_length_tuples(
    spreadsheet_document: Document,
) -> List[Tuple[float, float]]:
    return [
        (
...
def get_studs_diameter_length_tuples(
    spreadsheet_document: Document,
) -> List[Tuple[float, float]]:
    return [
        (
        ...
        (
            spreadsheet_document.Alternator.JackingRodDiameter,
            jacking_rods_length * spreadsheet_document.Alternator.NumberOfJackingHoles,
        ),
        (
            spreadsheet_document.Alternator.CoilWinderCenterRodDiameter,
            spreadsheet_document.Alternator.CoilWinderCenterRodLength
...
def sum_angle_bar_length(spreadsheet_document: Document) -> float:
    ...
    if wind_turbine_shape == WindTurbineShape.T:
        ...
        return sum(
            [
                round(spreadsheet_document.Alternator.StarShapeTwoHoleEndBracketLength)
                * 2,
...
def table(children: List[Element]) -> Element:
    ...
    return {"tagName": "table", "children": children}


def thead(children: List[Element]) -> Element:
    """https://developer.mozilla.org/en-US/docs/Web/HTML/Element/thead"""
    return {"tagName": "thead", "children": children}


def tbody(children: List[Element]) -> Element:
    """https://developer.mozilla.org/en-US/docs/Web/HTML/Element/tbody"""
    return {"tagName": "tbody", "children": children}


def tfoot(children: List[Element]) -> Element:
    """https://developer.mozilla.org/en-US/docs/Web/HTML/Element/tfoot"""
    return {"tagName": "tfoot", "children": children}


def tr(children: List[Element]) -> Element:
    """https://developer.mozilla.org/en-US/docs/Web/HTML/Element/tr"""
    return {"tagName": "tr", "children": children}


def td(
    content: Any = None, col_span: Optional[int] = None, row_span: Optional[int] = None
) -> Element:
...
def tcell(
    ...
    element: Element = {"tagName": tag_name, "properties": {}}
    if isinstance(content, dict):
        element["children"] = [content]
    else:
        element["properties"] = {"textContent": "" if content is None else str(content)}
...
def img(src: str, alt: str, style: str = "") -> Element:
    ...
    element: Element = {
        "tagName": "img",
        "properties": {"src": src, "alt": alt, "style": style},
    }
    return element
...
def format_fastener(
    quantity: int, diameter: int, length: Optional[float] = None
) -> str:
