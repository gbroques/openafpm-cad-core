"""
Adapted from:
https://github.com/nephics/mat4py/blob/v0.6.0/mat4py/loadmat.py

Changes:
* Commented out lines for handling mxCHAR_CLASS:
...
def asbytes(s): return s.encode('latin1')
def asstr(b): return b.decode('latin1')
...
def read_struct_array(fd, endian, header):
    ...
    def empty(): return [list() for i in range(header['dims'][0])]
    array = {}
    for row in range(header['dims'][0]):
        for col in range(header['dims'][1]):
            for field in fields:
